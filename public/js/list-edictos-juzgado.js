...
function desplegarJuicios(id_materia, editar = null, id_juicio = null){
  ...
  $.ajax({
    method: 'GET',
    url: '/juicios-por-materia/'+id_materia,
  })
  .done(function( data ) {
...
$(document).ready(function () {
  ...
  modalConfirm(function(confirm){
    if(confirm){
      $.ajax({
       type:'GET',
       url: '/delete-edicto/'+idpublicacion
     })
      .done(function( msg ) {
        table.ajax.reload();
        Toast.fire({
  ...
  $('#edictos').on( 'click', '#btn-agregar', function () {
    ...
    //Se deshabilitan los campos distrito y juzgado
  ...
  $('#edictos-form').on('submit', function(e) {
      ...
      if(id_edicto_editar){
          $.ajax({
           ...
           method: "POST",
           url: '/update-edicto/'+id_edicto_editar+'/'+modificar_archivo,
           data: formData,
           contentType: false,
           processData: false})
          .done(function (msg) {
